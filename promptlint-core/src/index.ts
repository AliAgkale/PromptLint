...
export function analyze(text: string, options: AnalyzeOptions = {}): AnalysisResult {
  ...
  const obsAll    = runAllObservations(text, disabledRules, undefined, cheapestInputRate);
  const score     = scorePrompt(text, obsAll, tokens);
...
export { runAllObservations } from './analyzers/observations.js';
